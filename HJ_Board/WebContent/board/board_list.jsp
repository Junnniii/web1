	 ...
	 <div id="infoArea">
	 	...
	 	<caption> 게시판 리스트</caption>
	 		<c:choose>
	 			...
	 			<thead>
	 			<tr>
	 				<th scope="col" class="bssNumber">번호 </th>
	 				<th scope="col" class="bssTitle">제목</th>
	 				<th scope="col" class="bssAuthor">글쓴이</th>
	 				<th scope="col" class="bssDate">등록일</th>
	 				<th scope="col" class="bssHit">조회수</th>
	 				...
	 				<tbody>
	 					...
	 					<td><c:out value="${board_read_count}"/></td>	
	 					</tr>
	 				</tbody>
	 				</c:forEach>
	 			</c:when>
	 		...
	 		<td colspan="5">
	 			...
	 			<c:forEach var="start" begin="${startpage}" end="${endpage}" step="1">
	 				<c:choose>
	 					...
	 					<c:otherwise>
	 					<a href="./BoardList.do/page=<c:out value="${start}"/>">
	 					[<c:out value="${start}"/>]
	 					</a>&nbsp;
	 					</c:otherwise>
	 				</c:choose>
